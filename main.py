...
def convert_to_png(input_path, output_dir):
    ...
    if ext == ".pdf":
        ...
        if not os.path.exists(output_path):
            raise ValueError(
                "Failed to convert DWG/DXF file. Ensure 'dwg2png' is installed and in PATH."
            )
        ...
        raise ValueError(
            f"Unsupported file format: {ext}. Please provide a valid blueprint file."
        )
...
def extract_black_and_non_black_lines(
    image_path, output_dir, contrast_factor=2.0, black_threshold=50, dilation_size=5
):
    ...
    kernel_thick = cv2.getStructuringElement(
        cv2.MORPH_RECT, (dilation_size, dilation_size)
    )
    ...
    black_lines_array[
        (black_lines_array[:, :, 0] == 0)
        & (black_lines_array[:, :, 1] == 0)
        & (black_lines_array[:, :, 2] == 0)
    ] = [255, 255, 255, 0]
...
def process_image_for_dark_lines(image_path, scale_factor, output_dir):
    ...
    black_lines, non_black_lines = extract_black_and_non_black_lines(
        os.path.join(output_dir, "flipped.png"), output_dir
    )
    ...
    thin_lines_closed = cv2.morphologyEx(
        thin_lines, cv2.MORPH_CLOSE, kernel, iterations=2
    )
    ...
    contours, _ = cv2.findContours(
        thick_lines, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
    )
    contour_data = []
    for i, contour in enumerate(contours):
        scaled_coordinates = [
            (point[0][0] * scale_factor, point[0][1] * scale_factor)
...
def process_image(image_path, scale_factor, output_dir):
    ...
    approx_contours = [
        cv2.approxPolyDP(c, 0.005 * cv2.arcLength(c, True), True)
...
def shift_coordinates(contours, centroid):
    ...
    for contour in contours:
        shifted_coords = [
            (x - centroid[0], y - centroid[1]) for x, y in contour["coordinates"]
        ]
...
def remove_smaller_floors():
    floor_objects = [
        ...
        if obj.name.startswith("Floor") and obj.type == "MESH"
    ]
...
def create_door(
    ...
    thickness=0.35,
    rotation=0,
    texture_path="./assets/furniture/wood.jpg",
):
    location = (location[0], location[1], height / 2)
    bpy.ops.mesh.primitive_cube_add(
        location=location, scale=(width, thickness, height / 2)
    )
    ...
    bpy.ops.mesh.primitive_cylinder_add(
        ...
        radius=0.2,
        depth=thickness * 4,
        rotation=(0, 1.5708, handle_twist),
    )
    ...
    handle_links.new(
        handle_principled_node.outputs["BSDF"], handle_output_node.inputs["Surface"]
    )
...
def create_stairs(
    ...
    bpy.ops.mesh.primitive_cylinder_add(
        location=location, radius=radius, depth=central_pole_height
    )
    ...
    links.new(
        pole_texture_node.outputs["Color"], pole_principled_node.inputs["Base Color"]
    )
    ...
    for i in range(num_steps):
        ...
        bpy.ops.mesh.primitive_cube_add(
            location=(x, y, z), scale=(step_width, step_thickness, step_height / 2)
        )
        ...
        step_links.new(
            step_texture_node.outputs["Color"],
            step_principled_node.inputs["Base Color"],
        )
        step_links.new(
            step_principled_node.outputs["BSDF"], step_output_node.inputs["Surface"]
        )
...
def create_window(location, material, height=4, width=2, thickness=0.2, rotation=0):
    ...
    bpy.ops.mesh.primitive_cube_add(
        location=top_block_location, scale=(width, thickness, height / 8)
    )
    ...
    bpy.ops.mesh.primitive_cube_add(
        location=bottom_block_location, scale=(width, thickness, height / 8)
    )
    ...
    bpy.ops.mesh.primitive_cube_add(
        location=middle_pane_location, scale=(width, thickness, height / 4)
    )
...
def main():
    try:
        parser = argparse.ArgumentParser(
            description="Process blueprint and generate 3D model."
        )

        group = parser.add_mutually_exclusive_group(required=True)
        group.add_argument(
            "--convert", action="store_true", help="Convert blueprint to 3D model."
        )
        group.add_argument(
            "--stack", action="store_true", help="Stack multiple models."
        )

        parser.add_argument(
            "image", type=str, nargs="?", help="Path to blueprint image."
        )
        parser.add_argument(
            "--scale_factor",
            type=float,
            default=0.1,
            help="Scale of building (default: 0.1).",
        )
        parser.add_argument(
            "--wall_height",
            type=float,
            default=10,
            help="Height of the wall (default: 10).",
        )
        parser.add_argument(
            "--wall_thickness",
            type=float,
            default=0.5,
            help="Thickness of the wall (default: 0.5).",
        )
        parser.add_argument("--wall_texture", type=str, help="wall texture.")
        parser.add_argument("--floor_texture", type=str, help="floor texture.")
        parser.add_argument(
            "--output_directory", type=str, required=True, help="Path to write outputs."
        )
        ...
        if args.convert:
            ...
            floor_texture_path = resource_path(
                "./assets/floor/" + args.floor_texture.strip().strip("'")
            )

            if not os.path.exists(floor_texture_path):
                raise FileNotFoundError(
                    f"The floor texture file '{floor_texture_path}' does not exist."
                )
            ...
            floor_material = create_material_with_texture(
                "FloorTexture", floor_texture_path
            )

            wall_texture_path = resource_path(
                "./assets/wall/" + args.wall_texture.strip().strip("'")
            )

            if not os.path.exists(wall_texture_path):
                raise FileNotFoundError(
                    f"The wall texture file '{wall_texture_path}' does not exist."
                )
            ...
            wall_material = create_material_with_texture(
                "WallTexture", wall_texture_path
            )
            ...
            for contour in shifted_contours:
                floor = create_floor_with_texture(
                    contour["coordinates"], floor_material
                )

            remove_smaller_floors()

            json_path = process_image_for_dark_lines(
                converted_path, args.scale_factor, output_dir
            )
            ...
            for contour in shifted_contours:
                wall = create_walls_with_texture(
                    contour["coordinates"], args.wall_height, wall_material, thick
                )
            ...
            object_detection(
                flip_image_path,
                model_path=resource_path("./assets/model/model.pt"),
            ...
            for detection in detections:
                ...
                if class_name == "door":
                    create_door(
                        location=(shifted_x, shifted_y),
                        height=args.wall_height,
                        thickness=args.wall_thickness / 2,
                        width=(min(dim1, dim2) / 2) * args.scale_factor,
                        rotation=0 if rotation else rotation,
                        texture_path=resource_path("./assets/furniture/wood.jpg"),
                    )
                    ...
                    create_window(
                        ...
                        width=((max(dim1, dim2) + 20) / 2) * args.scale_factor,
                        rotation=rotation,
                    )
                    ...
                    create_double_door(
                        ...
                        height=args.wall_height,
                        thickness=args.wall_thickness / 2,
                        width=(max(dim1, dim2) / 2) * args.scale_factor,
                        rotation=rotation,
                        texture_path=resource_path("./assets/furniture/wood.jpg"),
                    )
                    ...
                    create_stairs(
                        ...
                        radius=1,
                        step_height=0.4,
                        step_width=((min(dim1, dim2) / 2) * args.scale_factor) / 3,
                        step_thickness=0.8,
                        texture_path=resource_path("./assets/furniture/wood.jpg"),
                    )

            bpy.ops.wm.save_as_mainfile(
                filepath=(os.path.join(output_dir, "furnished.blend"))
            )
